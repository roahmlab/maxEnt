function [prog,Q,phi,y,basis] = buildTrigSOSDecomp(prog,q,c,s,mults)
%
...
%
    ...
    for i = 1:length(M)
        [tossprog,m(i)] = tossprog.newFreePoly(M{i});
    end
    ...
    sosCnst = mss_standardize_trig_power(q-phi'*Q*phi,c,s);
    
    A = diff(sosCnst,freevar);
    b = subs(sosCnst,freevar,0*freevar);
    
    [svar,spow,sCoeff] = decomp([b A].');
    [prog,y] = prog.withEqs(sCoeff.'*[1;freevar]);
    basis = recomp(svar,spow,eye(size(spow,1)));    
...
%     [prog,yreal] = prog.withEqs(A*[1;freevar]);
    
%     % First, map indices of yreal to indices of orig.
%     ycmp = msspoly(zeros(2*size(Coeff,2),1));

%     ycmp(find(~J)) = yreal;
%     y = ycmp(1:size(Coeff,2),1) + sqrt(-1)*ycmp(size(Coeff,2)+(1:size(Coeff,2)),1);
    
%     zbasis = recomp(var,pow,eye(size(pow,1)));
%     basis = subs(zbasis,z,c+sqrt(-1)*s);
%     keyboard
end